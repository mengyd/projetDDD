...
public class TestPlanifierEntretien extends TestCase {
	...
	//planifier
		@Test
		public void test_should_return_entretien() {		
			
			Candidat candidat = new Candidat("Dubois", "Cedric", getListTechnos(Technos.JAVA, Technos.CSHARP) ,5);
			Creneau creneau = new Creneau();
			Entretien entretien = new Entretien(getListConsultants(), candidat, creneau);
			
			PlanifierEntretien planifierEntretien = new PlanifierEntretien();
			Assert.assertEquals(entretien, planifierEntretien.planifier(getListConsultants(), candidat, creneau));
